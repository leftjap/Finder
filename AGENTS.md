...
- 이 프로젝트의 빌드 출력물은 `Explorer.exe`이다. Windows 시스템 셸도 `explorer.exe`이다.
- **절대로** `Stop-Process -Name Explorer`, `taskkill /IM Explorer.exe`, 또는 이름 기반 프로세스 종료 명령을 실행하지 않는다. 실행 시 Windows 작업표시줄·바탕화면·키보드 입력이 모두 중단된다.
- 빌드 전 앱 종료가 필요하면 `사용자: Explorer.exe 앱 창을 닫아주세요`라고 안내하고 사용자 확인을 기다린다.
