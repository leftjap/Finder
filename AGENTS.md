...
빌드 명령 (Git Bash):
```bash
cd /c/dev/apps/finder && pyinstaller Finder.spec --distpath dist/staging && rm -rf bin.pending && mv dist/staging/Finder bin.pending && rm -rf dist/staging && if [ ! -f bin/Finder.exe ]; then rm -rf bin; mv bin.pending bin; echo ">>> bin.pending → bin 직접 반영 (bin 부재)"; fi
```
⚠️ `cmd //c build.bat` 사용 금지 — Claude Code는 Git Bash를 사용하므로 cmd.exe CWD 미상속 문제 발생

출력 경로: `C:\dev\apps\finder\dist\staging\Finder\` → `C:\dev\apps\finder\bin.pending\` 로 이동
사용자 실행 경로: `C:\dev\apps\finder\bin\Finder.exe` (작업표시줄 고정 대상)
빌드는 앱 실행 중에도 완료됨 (pending 폴더에 이동). 반영은 앱 재실행 시 자가 업데이트로 적용.
