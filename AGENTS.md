...
robocopy dist\Finder C:\apps\Finder /MIR /NJH /NJS
```

출력 경로: `C:\dev\apps\finder\dist\Finder\Finder.exe` → `C:\apps\Finder\Finder.exe` 로 자동 복사
사용자 실행 경로: `C:\apps\Finder\Finder.exe` (작업표시줄 고정 대상)
코드 수정은 앱 실행 중에도 가능. 빌드 시에만 앱 종료 필요.
...
- 빌드 전 앱 종료가 필요하면 `사용자: C:\apps\Finder\Finder.exe 앱 창을 닫아주세요`라고 안내하고 사용자 확인을 기다린다.
